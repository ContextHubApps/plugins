---
description: Catch up on what you have been working on in ContextHub, and pick the thread back up
argument-hint: [topic]
allowed-tools: mcp__contexthub__list_my_recent_work, mcp__contexthub__list_org_folders, mcp__contexthub__search_org_knowledge, mcp__contexthub__read_org_doc
---
...
Call `mcp__contexthub__list_my_recent_work`. Default window is 30 days; pass `days` only if they asked about something older.
...
If their activity is all in `product/` and `$1` is about a deployment runbook, scoping `mcp__contexthub__search_org_knowledge` to `product` guarantees you miss it — and you will never find out, because a search returning nothing looks exactly like a subject nobody wrote about.
...
Call `mcp__contexthub__read_org_doc` on the ones that match what they asked, not the ones that are merely most recent. If nothing looks right, say what you found and ask — that is faster for them than watching you open six files that turn out to be wrong.
...
- Do not call `mcp__contexthub__list_my_recent_work` repeatedly hunting for a better answer.
